...
import it.vige.businesscomponents.transactions.RepeatableReadTestCase.RepeatableReadTestCaseSetup;
...
@ServerSetup(RepeatableReadTestCaseSetup.class)
public class RepeatableReadTestCase {

	private static final Logger logger = getLogger(RepeatableReadTestCase.class.getName());

	public static final String TEST_DS_XML = "repeatableread-ds.xml";
	...
	public static JavaArchive createJavaDeployment() {
		final JavaArchive jar = create(JavaArchive.class, "repeatableread-test.jar");
		jar.addPackage(MyCallableTask.class.getPackage());
		jar.addClass(Account.class);
		jar.addAsManifestResource(new FileAsset(new File("src/test/resources/isolations/repeatableread-test.xml")),
				"persistence.xml");
	...
	static class RepeatableReadTestCaseSetup implements ServerSetupTask {
