...
import it.vige.businesscomponents.transactions.SerializableTestCase.SerializableTestCaseSetup;
...
@ServerSetup(SerializableTestCaseSetup.class)
public class SerializableTestCase {

	private static final Logger logger = getLogger(SerializableTestCase.class.getName());

	public static final String TEST_DS_XML = "serializable-ds.xml";
	...
	public static JavaArchive createJavaDeployment() {
		final JavaArchive jar = create(JavaArchive.class, "serializable-test.jar");
		jar.addPackage(MyCallableTask.class.getPackage());
		jar.addClass(Account.class);
		jar.addAsManifestResource(new FileAsset(new File("src/test/resources/isolations/serializable-test.xml")),
				"persistence.xml");
	...
	static class SerializableTestCaseSetup implements ServerSetupTask {
